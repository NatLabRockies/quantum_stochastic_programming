...
% ------------------------------------------------------------------------------
\keyblock{Key Theoretical Findings}{
\setlength{\itemsep}{6pt}
\begin{itemize}
  \item \textbf{Polynomial speedup:} QAE estimates $\phi(x)$ with error
    $O(1/\varepsilon)$ circuit calls vs $O(1/\varepsilon^2)$ for classical Monte
    Carlo --- a provable polynomial advantage assuming efficient distribution
    preparation.
  \item \textbf{Parallel scenario optimisation:} Encoding $p(\xi)$ as a quantum
    wavefunction and using it as control logic for the phase-separation unitary
    $H_Q$ lets DQA converge to the optimal dispatch $y_\omega^*$ for
    \emph{all} $2^{n_\xi}$ scenarios simultaneously in a single quantum circuit.
  \item \textbf{Controllable error:} Two independent error sources can each
    be made arbitrarily small --- DQA residual $\delta\!\sim\!O(1/T^2)$ via
    annealing time $T$, and QPE error $O(\pi/M)$ via register size $M$.
    Setting $T\!=\!n_y^2$ suffices empirically.
  \item \textbf{Complexity separation:} Runtime scales as $O(2^{n_y})$ in
    annealing time (second-stage turbines) but is \emph{independent} of the
    number of scenarios $|\Omega|\!=\!2^{n_\xi}$, breaking the classical
    $O(|\Omega|)$ bottleneck.
  \item \textbf{Power grid demonstration:} Algorithm validated on a stochastic
    unit commitment problem (gas commitment under wind uncertainty), confirming
    that DQA+QAE recovers the classical expected cost with controllable
    accuracy.
\end{itemize}
}
